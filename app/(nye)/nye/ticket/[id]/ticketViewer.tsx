...
export function TicketViewer ({ ticket }) {
  ...
  return [
    <div
      key={1}
      id={'ticket'}
      className="relative bg-primary md:w-auto text-xs mt-auto font-medium mb-12">
      <div
        className="ticket-left text-left w-full h-full justify-center pl-7 pr-24 md:pr-44 md:pl-12 py-6 md:py-8 flex gap-2 md:gap-4 flex-col">
                <span className={'whitespace-nowrap'}>31.12.2023 HIDDE'S NYE COLDHOUSE<br/>
                  WARMINGBIRTHDAYPARTY</span>
        <h2
          className={'text-2xl md:text-5xl md:max-w-[20rem] break-word ' +
            (canvassing
              ? 'max-h-24'
              : 'line-clamp-2')}>{ticket.name}</h2>
        ...
        <div className="flex items-center gap-4">
          ...
          <h6 className={'text-sm font-medium leading-tight md:text-base'}>
             <span className={''}> {ticket.email.length > 0
      ...
      <div className="absolute h-full left-0 top-0">
        ...
        <span
          className="text-2xl md:text-3xl absolute -right-4 top-1/2 transform -translate-y-1/2 rotate-90">№{ticket.no}</span>
    ...
    <div key={3} className={'text-xl text-center'}>superleuk dat je erbij
      bent! <span
        className="text-purple-500">
          ☺
        </span>︎ groetjes hidde
    </div>,
    ticket.dinner ? <div key={4} className={'text-base'}>
      dieet wensen? <a href="https://wa.me/310623051810">let me know</a>
    </div> : null,
    <div className={'flex gap-4 flex-wrap'} key={5}>
      <button
        ...
        className={`btn text-center`}><span
        className="hs-icon link-arrow-right mr-1"></span> Share this NYE party
        with
        friends
      </button>
    </div>
